...
def pose_step(pose: np.ndarray, action: np.ndarray, dt: float, max_v: float, max_omega: float) -> np.ndarray:
...
def cell_of_xy(x: float, y: float, cell_size: float, grid_w: int, grid_h: int) -> Tuple[int, int]:
...
def point_in_rear_sector(attacker_pose: np.ndarray, target_pose: np.ndarray, view_range: float, half_angle: float) -> bool:
...
# target の背後をより忠実に表現
def camera_sees_target_rear(attacker_pose: np.ndarray, target_pose: np.ndarray, camera_range: float, half_angle: float) -> bool:
    attacker_xy = attacker_pose[:2]
...
def has_won(attacker_pose: np.ndarray, target_pose: np.ndarray, cfg) -> bool:
    return (
        point_in_rear_sector(attacker_pose, target_pose, cfg.rear_view_range, cfg.rear_sector_half_angle)
        and camera_sees_target_rear(attacker_pose, target_pose, cfg.camera_range, cfg.camera_fov_half_angle)
    )
