...
class MPCController:
    ...
    def act(self, self_pose: np.ndarray, other_pose: np.ndarray, path: List[np.ndarray]) -> Tuple[np.ndarray, dict]:
        ...
        for i, seq in enumerate(seqs):
            ...
            if cost < best_cost:
                best_cost = cost
                best_i = i
        return seqs[best_i, 0].copy(), {"cost": float(best_cost), "mean_cost": float(np.mean(costs))}
    ...
    def _rollout_cost(self, pose: np.ndarray, other_pose: np.ndarray, seq: np.ndarray, path: List[np.ndarray]) -> float:
        c = self.cfg
        p = pose.copy()
        cost = 0.0
        ...
        for t, u in enumerate(seq):
            ...
            cost -= 0.18 * max(float(u[0]), 0.0) / c.max_v  # prefer quick forward tracking
            if sep < danger_margin and float(u[0]) < 0.0:
                cost -= 0.75 * abs(float(u[0])) / max(c.max_v, 1e-6)  # allow deliberate retreat
            prev_action = u
