...
def blocked_cells_around(center_xy: np.ndarray, radius: float, cfg) -> set:
    cx, cy = cell_of_xy(
        float(center_xy[0]), float(center_xy[1]), cfg.cell_size, cfg.grid_w, cfg.grid_h
    )
...
def unblock_escape_neighbors(
    start: Cell, avoid_xy: np.ndarray, blocked: set, cfg
) -> None:
...
def rear_goal_cells(target_pose: np.ndarray, cfg, n_rings: int = 7) -> List[Cell]:
    ...
    for r in range(1, n_rings + 1):
        ...
        for da in np.linspace(
            -cfg.rear_sector_half_angle * 0.85, cfg.rear_sector_half_angle * 0.85, 9
        ):
...
def astar(
    ...
    if blocked is None:
        blocked = set()
    valid_goals = [
        g for g in goals if in_bounds(g, cfg.grid_w, cfg.grid_h) and g not in blocked
    ]
    if not valid_goals:
    ...
    while frontier:
        ...
        for dx, dy, step_cost in NEIGHBORS_4:
            ...
            if preferred_y is not None:
                ...
                lane_cost = (
                    0.015
                    * abs(float(nxt_y) - float(preferred_y))
                    / max(cfg.field_h, 1e-6)
                )
...
def plan_cells_to_rear(
    ...
    start = cell_of_xy(
        float(attacker_pose[0]),
        float(attacker_pose[1]),
        cfg.cell_size,
        cfg.grid_w,
        cfg.grid_h,
    )
...
def plan_path_to_rear(
    ...
    cells = plan_cells_to_rear(
        attacker_pose, target_pose, cfg, avoid_xy=avoid_xy, preferred_y=preferred_y
    )
...
def path_is_grid_aligned(path: List[np.ndarray], cfg, tol: float = 1e-8) -> bool:
    ...
    if not path:
        return True
    cells = [
        cell_of_xy(float(p[0]), float(p[1]), cfg.cell_size, cfg.grid_w, cfg.grid_h)
        for p in path
    ]
